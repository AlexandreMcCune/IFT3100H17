...
void Application::mousePressed(int x, int y, int button)
{
  ...
  if(renderer->isInside(x, y, 0, 0, renderer->framebufferWidth, renderer->framebufferHeight))
  {
    ...
    renderer->fatPixelStateChange(fatPixelIndex, PixelState::ON);
  }
...
void Application::mouseReleased(int x, int y, int button)
{
  ...
  if(renderer->isInside(x, y, 0, 0, renderer->framebufferWidth, renderer->framebufferHeight))
  {
    ...
    renderer->fatPixelStateChange(fatPixelIndex1, PixelState::START);
    renderer->fatPixelStateChange(fatPixelIndex2, PixelState::END);
  }
...
void Application::keyReleased(int key)
{
  ...
  {
    case 49:  // key 1
      renderer->lineRenderer = LineRenderer::DDA;
      ofSetWindowTitle("rastérisation de lignes (DDA)");
      ofLog() << "<draw mode: DDA>";
      break;

    case 50:  // key 2
      renderer->lineRenderer = LineRenderer::BRESENHAM;
      ofSetWindowTitle("rastérisation de lignes (Bresenham)");
