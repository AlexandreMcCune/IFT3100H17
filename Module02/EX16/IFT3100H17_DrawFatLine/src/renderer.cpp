// IFT3100H17_DrawLine/renderer.cpp
...
void Renderer::setup()
{
  ...
  lineRenderer = LineRenderer::BRESENHAM;
...
void Renderer::reset()
{
  ...
  for(index = 0; index < bufferCount; ++index)
  {
    ...
    fatPixels[index].state = PixelState::NONE;
...
void Renderer::fatPixelStateChange(int index, PixelState state)
{
  ...
  if(isVerbose)
    ofLog() << "<fat pixel index: " << index << " change to state: " << (int)state << ")>";
}
...
void Renderer::rasterLineDDA(float x1, float y1, float x2, float y2)
{
  ...
  float length = abs(abs(xDelta) >= abs(yDelta) ? xDelta : yDelta);

  if(length == 0.0f)
    return;

  float xIncrement = xDelta / length;
  float yIncrement = yDelta / length;
  ...
  for(int i = 1 ; i <= length ; ++i)
  {
    ...
    fatPixelStateChange(index, PixelState::ON);
...
void Renderer::rasterLineBresenham(float x1, float y1, float x2, float y2)
{
  ...
  if(error < 0)
    error /= 2;  // pour éviter le bit shifting à droite des valeurs négatives
  else
